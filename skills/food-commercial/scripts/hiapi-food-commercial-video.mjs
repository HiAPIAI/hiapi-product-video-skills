...
export const RECIPES = Object.freeze({
  "product-hero": {
    shot: "a tight commercial hero close-up",
    action: "one controlled turntable reveal",
    setting: "a clean uncluttered product counter",
    lighting: "crisp key light with appetizing edge highlights",
    texture: "real package materials and natural condensation",
    camera: "one slow push-in",
    sound: "restrained room tone and precise product handling",
  },
  "coffee-pour": {
    shot: "a macro three-quarter close-up",
    action: "one continuous pour into the cup",
    setting: "a clean cafe counter",
    lighting: "warm directional side light",
    texture: "real crema, steam, and liquid flow",
    camera: "one slow push-in",
    sound: "natural pour and cup ambience",
  },
  "beverage-splash": {
    shot: "a high-speed product close-up",
    action: "one contained splash around the container",
    setting: "a chilled studio surface",
    lighting: "bright backlight with crisp highlights",
    texture: "real fizz, droplets, ice, and condensation",
    camera: "one short lateral slide",
    sound: "one crisp splash with natural fizz",
  },
  "food-macro": {
    shot: "an extreme food macro",
    action: "one slow finishing drizzle over the dish",
    setting: "a clean plated surface",
    lighting: "soft directional food light",
    texture: "truthful doneness, crumb, sauce, and moisture",
    camera: "one slow macro push-in",
    sound: "subtle kitchen ambience",
  },
  "restaurant-atmosphere": {
    shot: "an intimate table-level medium close-up",
    action: "one server places the featured dish on the table",
    setting: "a recognizable dining-room setting",
    lighting: "warm practical light with clean food highlights",
    texture: "fresh steam and truthful plating",
    camera: "one slow dolly-in",
    sound: "restrained dining-room ambience",
  },
});
...
const REPEAT_FLAGS = new Map([
  ["--hero-image-url", "heroImageUrls"],
  ["--hero-image-file", "heroImageFiles"],
  ["--reference-image-url", "referenceImageUrls"],
  ["--reference-image-file", "referenceImageFiles"],
]);
...
export function usage() {
  return `HiAPI Food Commercial Video

Usage:
  node scripts/hiapi-food-commercial-video.mjs --recipe RECIPE --prompt TEXT [options] --preview
  node scripts/hiapi-food-commercial-video.mjs --recipe RECIPE --prompt TEXT [options] --dry-run
  node scripts/hiapi-food-commercial-video.mjs --recipe RECIPE --prompt TEXT [options] --spend --approved-request-hash HASH
  node scripts/hiapi-food-commercial-video.mjs --task-id ID --env-file PATH
  node scripts/hiapi-food-commercial-video.mjs --check --env-file PATH

Recipes:
  product-hero | coffee-pour | beverage-splash | food-macro | restaurant-atmosphere

Media routes:
  No image                         Kling text-to-video
  One --hero-image-file/url        Kling image-to-video
  1-9 --reference-image-file/url   Seedance 2.0 Fast

Options:
  --duration SECONDS              Kling: integer 3-15; Seedance Fast: 4-15. Default: 4
...
function hasRequestInput(options) {
  return Boolean(
    options.recipe || options.prompt || options.action || options.setting || options.lighting || options.texture ||
    options.camera || options.sound || options.duration !== undefined || options.resolution || options.ratio || options.noAudio ||
    options.approvedRequestHash || options.idempotencyKey || options.heroImageUrls.length || options.heroImageFiles.length ||
    options.referenceImageUrls.length || options.referenceImageFiles.length
  );
}
...
export function buildCommercialPrompt(options, recipe, duration, audioEnabled) {
  ...
  const pacing = duration >= 9
    ? "use roughly 20% setup, 60% primary action, and 20% clean settle-and-hold"
    : "start the primary action promptly and reserve the final moment for a clean product hold";
  return [
    `Create a ${duration}-second single-shot food commercial.`,
    `Shot type: ${direction.shot}.`,
    `Product or dish truth: ${userPrompt}.`,
    ...
    `Appetite texture: ${options.texture?.trim() || direction.texture}.`,
    ...
    "Keep food texture, doneness, portion, garnish, visible ingredients, hands, utensils, cup rims, plates, and hygiene truthful and coherent.",
    "Add no captions, CTA, overlay text, or new logo. Leave clean negative space for deterministic post-production.",
  ].join("\n");
}
...
export async function buildRequest(options) {
  ...
  const audioEnabled = !options.noAudio;
  const heroImages = await resolveImages(options.heroImageUrls, options.heroImageFiles);
  const referenceImages = await resolveImages(options.referenceImageUrls, options.referenceImageFiles);
  if (heroImages.length && referenceImages.length) throw new Error("Do not mix hero images with reference images.");
  if (heroImages.length > 1) throw new Error("The hero image route accepts exactly one image.");
  if (referenceImages.length > 9) throw new Error("The reference image route accepts at most 9 images.");
  const duration = normalizeDuration(
    options.duration ?? 4,
    referenceImages.length > 0 ? 4 : 3,
    referenceImages.length > 0 ? "Seedance Fast" : "Kling",
  );

  const prompt = buildCommercialPrompt(options, recipe, duration, audioEnabled);
  if (heroImages.length === 1) {
    ...
    return {
      ...
      payload: {
        model: MODELS.hero,
        storage: "temp",
        input: { image_urls: heroImages, prompt, resolution, duration, sound: audioEnabled },
      },
    };
  }
...
export function idempotencyKeyForRequest(providedKey, hash) {
  const prefix = `food-${hash.slice(0, 12)}-`;
...
function qcTemplate() {
  const ids = [
    "package_geometry_and_brand_color",
    "food_texture_doneness_portion_and_ingredients",
    "liquid_steam_condensation_fizz_and_melting_physics",
    "hands_utensils_rims_and_contact_continuity",
    "hygiene_and_appetite_appeal",
    "claims_allergens_alcohol_certifications_and_health_language",
    "single_shot_mobile_framing_and_safe_space",
    "audio_synchronization_and_unintended_speech",
  ];
  return {
...
generated_unreviewed - human food, brand, claims, continuity, framing, and audio QC is required before publication.
`;
}
...
async function savePackage(options, built, task, estimatedCostUsd, deps = {}) {
  ...
  const manifest = {
    skill: "hiapi-food-commercial-video",
    recipe: built.recipe,
    route: built.route,
    model: built.payload.model,
    taskId: task.taskId,
...
function inferRecoveredRoute(model) {
  ...
  if (model === MODELS.hero) return "hero-image-to-video";
...
async function saveRecoveredPackage(options, task, deps = {}) {
  const route = inferRecoveredRoute(task.model);
  ...
  const built = {
    ...
    route,
    payload: { model: task.model, recovered: true },
